...
sub IMAPConnection
{
	...
	Timeout  => 60,
    Debug    => 0
	) or $BU->_log("Can not connect to ".$self->xget('/EDWEBS/IMAP/Server/node()')." with $@");
	return $imap;
}

sub IMAPConnectionTest
{
	my $self = shift;
	my $imap;
	my $BU = BundleUtils->new($self->{'LogFile'});
...
sub reqParserDB
{
	...
	my $DBH = $DBC->conn('db2','','TESTEDW',$self->xget("/EDWEBS/BuildAccountMap/TEST/\@DB2User"),$BU->decrypt($self->xget("/EDWEBS/BuildAccountMap/TEST/\@DB2Pass"),$ENV{BobKey},$ENV{BobKey}));
	my $SQL = qq{SELECT msgid,
						...
						subject,
						seen
				   FROM edwtbldr.bundle_build_request
				  WHERE seen IS NULL
	...
	if ($rc->[5] =~ /^(\[|FW:\[)BLD_REQ\]:(.*?)Raid=/gi)
	{
		...
		$mq->{MQCount}++;
		#update to seen
		$SQL = "update edwtbldr.bundle_build_request set seen='Y' where msgid=".$rc->[0];
		$sth = $DBH->prepare($SQL);
		$sth->execute();
	}
	#print Dumper $mq;	
...
sub raidValidation
{
    ...
    my $p4tickets = `p4 tickets|grep -i v-azhu`;
	`echo %P4PASSWD%|p4 login` if $? != 0;
	...
	foreach my $r (@raids)
	{
		...
		for ($ldw,$edw)
		{
            ...
            $BundleUtils->_log("verify $_ ...\n$output");
			close ( FILE );
            $BundleUtils->_log("[WARNING] : Please login perforce first.\n$output") if $output =~ /Perforce password (P4PASSWD) invalid or unset/i;
    ...
    $BundleUtils->_log("depots=@depots");
return @depots;
}
...
sub bLogger
{
	...
	for (;;)
	{
		...
		{
			sleep 300;
			$BundleUtils->_log("Facing issue to connect to TESTEDW server, sleepping 300 sec. and retry");
			next;
		} else
		{
			last;
...
pmrep connect -r %Infa_Repository% -d %Infa_Domain% -n %Infa_User% -X Infa_Pass
...
sub mvBundleFiles
{
	...
	{
		system("mv $file \\\\chc-filidx\\DSTest\\EDW\\RAIDS") if ((-d $file) and $file =~ /\d$/ and (-C $file) > $self->xget('/EDWEBS/Bundle/MaxkeepDays/node()'));
	}
}
...
sub PWDExpire
{
	...
	my $WarnStartDate = UnixDate(DateCalc($PWDLastSetDate,"+85 days"),"%D\n");
	...
	if (Date_Cmp($CurrentDate,$WarnStartDate) > -1)
	{
		my $message = MIME::Lite->new(
			From    => 'edwbld@expedia.com',
			To      => $self->xget('/EDWEBS/Alert/Reciever/text()'),
			Subject => '[ALERT] Password for s-edwdeploy/edwdeploy will be expiring in next few days. Please reset it ASAP, PasswordLastSet is '.$PWDLastSetDate.'<oef>',
			Type    => 'multipart/mixed',
		);
		$message->attach(
			Type     => 'text/html',
