...
while (1)
{
    $BU->_log("Bob running as DevMode, Bundle will be created from //depot/Tools/dev_dw branch") if $BC->xget('/EDWEBS/Bundle/DevMode/@IsEnabled') =~/yes/i;
	...
	if (defined($ActiveQueue))
	{
		...
		foreach my $AQKey(@AQKeys)
		{
			if ( $ActiveQueue->{$AQKey}->{BundleStatus} ne 'Running' )
			{
				...
				$BC->sendMsg($AQKey,'RequestType',$ActiveQueue->{$AQKey}->{Subject}->{RequestType});
				$BC->sendMsg($AQKey,'Cmd',$ActiveQueue->{$AQKey}->{Subject}->{Cmd});
				$BC->bundlebuildAutoUpd(($ActiveQueue->{$AQKey}->{Subject}->{_DevMode_}=~/yes/i or $BC->xget('/EDWEBS/Bundle/DevMode/@isEnabled') =~/yes/i)?'dev_dw':'dw'); # BundleBuild auto update
				...
				if ( lc($ActiveQueue->{$AQKey}->{Subject}->{Stage}) ne "only" )
				{
					...
					if ( lc($ActiveQueue->{$AQKey}->{Subject}->{RequestType}) eq "deploy" )
                    {
                        $cmd = "start ".(($ActiveQueue->{$AQKey}->{Subject}->{_DevMode_}=~/yes/i or $BC->xget('/EDWEBS/Bundle/DevMode/@isEnabled') =~/yes/i)?'_antins_':'antins')." ".$AQKey;
                    ...
                    } elsif ( lc($ActiveQueue->{$AQKey}->{Subject}->{RequestType}) eq "runcmd" )
                    {
                        $cmd = "start plugins.cmd ".$AQKey;
                        $cmd .= " ".$ActiveQueue->{$AQKey}->{Subject}->{raids};
                        $cmd .= " ".$ActiveQueue->{$AQKey}->{BundleName};
                        $cmd .= " ".$ActiveQueue->{$AQKey}->{Subject}->{ENV};
                        $cmd .= " \"$ActiveQueue->{$AQKey}->{Subject}->{Cmd}\"";
                    }
					$BU->_log($cmd);
					system($cmd);
	...
	$BU->_log("sleeping ".$BC->xget('/EDWEBS/Bundle/SleepTime/node()')." sec. and recheck new one") if ( $ActiveQueue->{MQCount} > 0 );
