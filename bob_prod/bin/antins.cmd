...
perl %_BobRoot%\bin\BundleBuild.pl -raids %_Raids% -triagegroup edw -debug -ignorever> %_BundleRootDir%\StageLogs\CBundle%_BundleName%.txt 2>&1
...
perl %_BobRoot%\bin\BundleBuild.pl -raids %_Raids% -debug > %_BundleRootDir%\StageLogs\CBundle%_BundleName%.txt 2>&1
@IF /I "%3" == "get" goto get
@if %errorlevel% neq 0 ( 
call perl -e "use lib '%_BobRoot%/lib';use BundleComm; my $BC = BundleComm->new('','','%_BobRoot%\\conf\\CntlFile.pcf');$BC->sendMsg(%_MsgID%,'CreateBundle','Failed');" 
goto problem )
...
@IF /I "%env%" == "DEV" call perl -e "use lib '%_BobRoot%/lib';use BundleComm; my $BC = BundleComm->new();$BC->infaPrepUpd(\"%_BundleName%\", \"%_InfaUser%\", \"%_InfaPass%\");"
