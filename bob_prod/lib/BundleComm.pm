...
#               2014-05-16 Added action handler and time window.
#               2014-05-22 performance tuning for permission check
#               2014-07-25 98480 remove db2 bundle logs
#               2015-03-20 101338 add runs cmd via bob
...
sub getAccountMap
{
	...
	my $env = uc(shift);
	my $BU = BundleUtils->new($self->{'LogFile'});
	my $envstr = "set env=$env\nset envName=".$self->xget("/EDWEBS/BuildAccountMap/$env/\@envName");
    $envstr .= "\nset repository=".$self->xget("/EDWEBS/BuildAccountMap/$env/\@repository");
...
sub alert
{
	...
	my $message = MIME::Lite->new(
		...
		Subject => '[ALERT] Bob on '.$ENV{computername}.' died,please investigating!',
		Type    => 'multipart/mixed',
	);
...
sub reqList
{
	...
	for ( $self->getMsgIDbyEnv($aq->{ENV}) )
	{
		...
		$rq->{$_}->{WhereItIs} =~ s/-p(.*?)\s-/-p ****** -/gi if $rq->{$_}->{WhereItIs} =~ /pmcmd\.exe/gi;
...
sub cancel
{
    ...
    my $server = $self->xget('/EDWEBS/IMAP/Folder/node()');
	if ( defined ($msgRef) )
	{
	...
	} else
	{
		...
		$imap->select($server) or die($BU->_log("Cannot open folder with $@"));
		$imap->set_flag("seen",$msg);
		$imap->move($self->xget('/EDWEBS/IMAP/Folder/node()')),$msg);
		$imap->expunge;
		$imap->logout;
	}
}
...
sub IMAPConnection
{
	...
	Timeout  => 60,
    #Debug    => 1
	) or $BU->_log("Cannot connect to ".$self->xget('/EDWEBS/IMAP/Server/node()')." with $@");
	return $imap;
}
...
{ 
  ...
  for (;;)
  {
	...
	{
		sleep 300;
		$BU->_log("Facing issue to connect to exchange server, sleepping 300 sec. and retry");
		next;
	} else
	{
		last;
  ...
  foreach my $msg(@msg)
  {
	...
	$alias = $1 if $hmsg->{From}->[0] =~ /<(.*?)\@/ig; 
    #$hmsg->{Subject}->[0] =~ s/\s//gi;
    $BU->_log("subject=".$hmsg->{Subject}->[0]);
  ...
  #$mq = $self->reqParserDB if not defined ( $mq );
...
sub reqParserDB
{
	...
	my $DBH = $DBC->conn('db2','','PRODEDW',$self->xget("/EDWEBS/BuildAccountMap/PROD/\@DB2User"),$BU->decrypt($self->xget("/EDWEBS/BuildAccountMap/PROD/\@DB2Pass"),$ENV{BobKey},$ENV{BobKey}));
	my $SQL = qq{SELECT msgid,
					    from,
						to,
						cc,
						to_char (receiveddate, 'DD MON YYYY HH:MI:SS') receiveddate,
						subject,
						seen
				   FROM dbadm.bundle_build_request
				  WHERE seen IS NULL
	...
	$sth->execute();
	my $rc = $sth->fetchrow_arrayref;
	
	$alias = $1 if $rc->[1] =~ /(.*?)\@/ig;
	$rc->[5] =~ s/\s//gi;
	if ($rc->[5] =~ /^(\[|FW:\[)BLD_REQ\]:(.*?)Raid=/gi)
	{
		if ($rc->[5] =~ /env=PROD/gi)
		{
			$sbj = $self->subjectParser($rc->[5]);		
			$mq->{$rc->[0]} = {
							Subject      => $sbj,
							From         => $rc->[1],
							To           => $rc->[2],
							Cc           => $rc->[3],
							ReceivedDate => $rc->[4]=~/,\s*(.*?)\s*[-|+]/?$1:$rc->[4]
						  };
			#$mq->{$msg}->{Body} = $imap->body_string($msg);
			@raids = split ":",$sbj->{raids};
			$mq->{$rc->[0]}->{BundleName} = $raids[0].'_'.$alias.'_'.$sbj->{ENV}.'_'.$BU->fdt('yyyymmddhh24miss');
			#$mq->{$msg}->{BundleName} = $msg;
			$mq->{$rc->[0]}->{ErrorCode} = $self->checkPermission($alias,$sbj->{ENV},'Deploy');		
			$mq->{MQCount}++;
			#update to seen
			$SQL = "update dbadm.bundle_build_request set seen='Y' where msgid=".$rc->[0];
			$sth = $DBH->prepare($SQL);
			$sth->execute();
		} else { next;}
	}
...
sub subjectParser
{
	...
	$subject = $1 if ($subject =~ /\[BLD_REQ\]:(.+?)($)/ig);
	@TmpArr=split /\|/,$subject;
	my $BU = BundleUtils->new($self->{LogFile});
	for (@TmpArr)
	{        
		$SubjRef->{raids}=$1 if ($_=~ /raid\s*=\s*([\S]+)(\||$)/i);
		$SubjRef->{ENV}=$1 if ($_=~ /ENV\s*=\s*([\S]+)(\||$)/i);
		...
		$SubjRef->{Cmd}=$1 if ($_=~ /cmd\s*=\s*\"(.+?)\"/is);
		$SubjRef->{_DevMode_}=$1 if ($_=~ /_DevMode_=(\w+)/i);
        #$SubjRef->{ErrorCode} = 'EDWEBS-00106' if not ( lc($SubjRef->{ENV}) ~~ ['dev','test','rc','mln','ppe','prod'] );        
	} 
    if (defined($SubjRef->{Cmd}))
    {
        $SubjRef->{RequestType}="RunCmd";
        $SubjRef->{ErrorCode} = $self->syntaxChecker($SubjRef->{ENV},"$SubjRef->{Cmd}");
    } else
    {
        $SubjRef->{RequestType}="Deploy";
        ...
        $BU->_log("Raids=@Raids"."RaidsCount=".$#Raids);
        ...
        #verify to see if InfaUser/InfaPass given on DEV.
        for (split ':',$SubjRef->{raids})
        {
            my $cl = `cl -c $_`;
...
sub syntaxChecker
{
    ...
    my $env=shift;
    my $cmd=shift;
    # we wont acturally check syntax of command rather than deciding if it is accepted command by compare with its pattern, If there are
    # syntax, let OS/Application error it out with messages. Maybe in the future we can do the real syntax check if needed.
    # if it is valid command and it can be runs on specified envrionment, return 0
    # if it is valid command but it can not be runs on specified envrionemnt, return 'EDWEBS-100101'
    # if it is unallowed command, return 'EDWEBS-100100'
    my $xp = XML::XPath->new(filename => $self->{'XConfig'});
    my $pattern;
    my @allowedEnvs;
    my $rc='EDWEBS-10100';
    foreach my $element ($xp->findnodes('/EDWEBS/CmdSyntaxPattern/*'))
    {
        $pattern = $element->string_value;
        if ($cmd =~ /$pattern/ig)
        {
            @allowedEnvs = split /\|/, $element->getAttribute("scope");
            if ( uc($env) ~~ @allowedEnvs )
            {
                $rc = 0;
                last;
            } else
...
sub getMsg
{
	...
	for(@array)
	{
		if ($_ =~ /$Msgid/i)
		{
			...
			$MsgRef->{'StartAt'} = $1 if ($_=~ /StartAt=(.*+)/i);
			$MsgRef->{'RequestType'} = $1 if ($_=~ /RequestType=(.*+)/i);
			$MsgRef->{'Cmd'} = $1 if ($_=~ /Cmd=(.*+)/i);            
		}
	}
	untie @array;
	return $MsgRef;
...
sub msgTable
{
	...
	$f->flock;
	#push @array,"*"x60;
	for my $i ('Raids','From','To','Cc','ENV','ReceivedDate','PID','BundleName','BldStartDT','CreateBundle','Status','Stage','Integrate','ProcessID','StartAt','RequestType','Cmd')
	{
...
sub bLogger
{
	...
	for (;;)
	{
		eval {$DBH = $DBC->conn('sql','chexsqledw04','EDWDE','','')};
		if ($@)
		{
			sleep 300;
			$BundleUtils->_log("Facing issue to connect to PRODEDW server, sleepping 300 sec. and retry");
			next;
		} else
		{
			last;
	...
	# {
		# $sql = qq{update dbadm.Bundle_Build_Process_Hist
					 # set logs = nvl(logs,' ') \|\| '@BldLogs[$s..$e]'
				   # where processid = $processID};
		# $sth = $DBC->execSQL($DBH, $sql);
		# $sth->finish;
		# $BundleUtils->_log($sql) if $sth->err;
		# $s=$e+1;
	...
	#print "\n>>>Logs for Bundle $msg are all archived, you can query table dbadm.Bundle_Build_Process_Hist by processID: $processID for details\n";
...
sub PWDExpire
{
	my $self = shift;
    my $BU = BundleUtils->new($self->{'LogFile'});
	my $Account = 's-edwdeploy';
	#my $PWDLastSet = `DSQuery * "CN=$Account,OU=Service Accounts,OU=ITI UG,DC=SEA,DC=CORP,DC=EXPECN,DC=com" -Attr PwdLastSet|grep -o '[0-9]*'`;
	my $PWDLastSet = `DSQuery * "CN=$Account,OU=Service,OU=ExpediaAccts,DC=expeso,DC=com" -Attr PwdLastSet|grep -o '[0-9]*'`;
	my $epoch = int(($PWDLastSet-116444916000000000)/10000000);
	my $PWDLastSetDate = UnixDate("epoch $epoch","%D\n");
    $BU->_log("Password last updated date=".$PWDLastSetDate);
	my $CurrentDate = localtime;
	my $WarnStartDate = UnixDate(DateCalc($PWDLastSetDate,"+64 days"),"%D\n");
    $BU->_log("Warning start at = ".$WarnStartDate);
	if (Date_Cmp($CurrentDate,$WarnStartDate) > -1)
	{
		my $message = MIME::Lite->new(
			From    => 'edwbld@expedia.com',
			To      => $self->xget('/EDWEBS/Alert/Reciever/text()'),
			Subject => '[ALERT] Password for account s-edwdeploy on expeso domain will be expiring in next few days. Please reset it ASAP, PasswordLastSet is '.$PWDLastSetDate.'<oef>',
			Type    => 'multipart/mixed',
		);
		$message->attach(
			Type     => 'text/html',
    ...
    return UnixDate(ParseDate('today'),'%Q');
}
1;
