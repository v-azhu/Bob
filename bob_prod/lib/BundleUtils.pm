...
{
	my $self = shift;
	my $p4ver = `p4 files //depot/tools/dw/bundlebuild.pl`;
	my $localver = `perl -S BundleBuild.pl -v`;
	my $op;
	$p4ver = $1 if $p4ver =~ /BundleBuild.pl#(\d+)/;
	$localver = $1 if $localver =~ /(\d+)/;
	if ( $p4ver != $localver )
	{
		$self->_log("P4 has version $p4ver but local version is $localver");
		# print screen out to current directory with name BundleBuild.pl " bob\bin "
		$op = `p4 print -o BundleBuild.pl //depot/tools/dw/BundleBuild.pl#$p4ver`;
		if ( $op =~ /no file\(s\) at that revision/gi)
		{
			$self->_log("No such version of file exists on p4, version=$p4ver");
		} else
		{
			$self->_log("BundleBuild.pl synced! Current version is $p4ver.");
		}
	}
